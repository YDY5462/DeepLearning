...
class RNN_model(nn.Module):
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        h0 = torch.zeros(2, self.batch_size, self.lstm_dim).to(batch_input.device)  # 初始化 LSTM 隐藏状态
        c0 = torch.zeros(2, self.batch_size, self.lstm_dim).to(batch_input.device)  # 初始化 LSTM 细胞状态
