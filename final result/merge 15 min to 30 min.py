...
os.chdir('D:/论文2/ResLSTM/15/')

data_predict=[]
with open('250-predictions.csv',"r") as file:
...
#删掉前10行
data_predict=data_predict[5:len(data_predict)][:]
print(len(data_predict))
data_old_merge=[[] for i in range(int(len(data_predict)/2))]
for i in range(int(len(data_predict)/2)):
	for j in range(len(data_predict[0])):
		data_old_merge[i].append(data_predict[i*2][j]+data_predict[i*2+1][j])
print(len(data_old_merge))

data_original=[]
with open('250-Y_test_original.csv',"r") as file:
	#data = csv.reader(file, delimiter=",")
	for line in file:
		line=line.replace('"','').strip().split(',')
		line=[float(x) for x in line]
...
#删掉前10行
data_original=data_original[5:len(data_original)][:]
print(len(data_original))
data_original_merge=[[] for i in range(int(len(data_original)/2))]
for i in range(int(len(data_original)/2)):
	for j in range(len(data_original[0])):
		data_original_merge[i].append(data_original[i*2][j]+data_original[i*2+1][j])
